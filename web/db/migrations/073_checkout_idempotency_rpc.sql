-- =============================================================================
-- 073_CHECKOUT_IDEMPOTENCY_RPC.SQL
-- =============================================================================
-- AUDIT-106: Add idempotency key parameter to process_checkout function.
-- This allows storing the idempotency key in the invoice for duplicate prevention.
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.process_checkout(
    ...
    p_idempotency_key TEXT DEFAULT NULL  -- AUDIT-106: Optional idempotency key
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- AUDIT-106: Insert invoice with idempotency key
...
EXCEPTION
    -- AUDIT-106: Handle idempotency key constraint violation
    WHEN unique_violation THEN
        -- Check if it's the idempotency key constraint
        IF SQLERRM LIKE '%idx_invoices_idempotency%' OR SQLERRM LIKE '%idempotency%' THEN
            ...
            END IF;
        END IF;
        -- Re-raise for other unique violations
        RETURN jsonb_build_object(
            'success', false,
...
'SEC-024 + AUDIT-106: Atomic checkout with server-side price validation and idempotency.
Client-supplied prices are IGNORED. Idempotency key prevents duplicate orders on retry.
