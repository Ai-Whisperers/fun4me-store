...
export default function AppointmentsError({ error, reset }: ErrorProps) {
  useEffect(() => {
    Sentry.withScope((scope) => {
      scope.setTag('errorBoundary', 'appointments')
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4">
      <div className="w-full max-w-md text-center">
        ...
        <h1 className="mb-2 text-2xl font-bold text-gray-800">Error al cargar citas</h1>
        <p className="mb-6 text-gray-500">No pudimos cargar la información de las citas. Por favor, intenta de nuevo.</p>
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <Link href="../" className="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-5 py-2.5 font-medium text-gray-700 hover:bg-gray-50">
            <Calendar className="h-4 w-4" />Volver al portal
          </Link>
        </div>
      </div>
    </div>
